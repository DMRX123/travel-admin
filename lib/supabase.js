// lib/supabase.js - FINAL PRODUCTION VERSION WITH BUILD-TIME FALLBACKS
import { createClient } from '@supabase/supabase-js';

// Use fallback values during build time to prevent errors
const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://xeymfzwxjurwbezgwdef.supabase.co';
const supabaseAnonKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || 'sb_publishable_hugKMF1Hkj3WkC7GoAD3gA_vhCYVqbj';
const supabaseServiceKey = process.env.SUPABASE_SERVICE_ROLE_KEY;

// Log warning only at runtime when missing
if (!process.env.NEXT_PUBLIC_SUPABASE_URL && typeof window !== 'undefined') {
  console.warn('⚠️ Supabase URL not configured. Using fallback values.');
}
...
export const supabaseAdmin = supabaseServiceKey && supabaseServiceKey !== 'placeholder-key'
...
export const VEHICLE_TYPES = {
  BIKE: { id: 'bike', name: 'Bike', icon: '🏍️', baseFare: 20, perKm: 8, seating: 1, description: 'Quick & Economical' },
  AUTO: { id: 'auto', name: 'Auto', icon: '🛺', baseFare: 30, perKm: 12, seating: 3, description: 'Best for short distances' },
  CAR: { id: 'car', name: 'Car', icon: '🚗', baseFare: 60, perKm: 15, seating: 4, description: 'Comfortable for families' },
  SUV: { id: 'suv', name: 'SUV', icon: '🚙', baseFare: 80, perKm: 20, seating: 6, description: 'Spacious for groups' },
  LUXURY: { id: 'luxury', name: 'Luxury', icon: '🚘', baseFare: 120, perKm: 30, seating: 4, description: 'Premium experience' },
  TEMPO: { id: 'tempo', name: 'Tempo Traveller', icon: '🚐', baseFare: 150, perKm: 25, seating: 12, description: 'Perfect for groups' },
};
...
export const completeRide = async (rideId, driverId, paymentId = null) => {
  try {
    ...
    await supabase
      ...
      .eq('id', rideId);
    await supabase.rpc('update_driver_earnings', { p_driver_id: driverId, p_amount: driverEarning });
    await supabase
      .from('drivers')
...
export const cancelRide = async (rideId, userId, reason, cancelledBy = 'user') => {
  try {
    ...
    let refundAmount = 0, cancellationFee = 0;
...
export const addLoyaltyPoints = async (userId, points, description) => {
  try {
    ...
    await supabase
      .from('loyalty_points')
      .upsert({ user_id: userId, points: newPoints, tier, updated_at: new Date().toISOString() });
    await supabase
      .from('loyalty_transactions')
      .insert({ user_id: userId, points, type: 'earned', description, created_at: new Date().toISOString() });
...
export const updateRideStreak = async (userId) => {
  try {
    ...
    await supabase
      .from('ride_streaks')
      .upsert({ user_id: userId, current_streak: currentStreak, best_streak: bestStreak, updated_at: new Date().toISOString() });
...
export const sendOTP = async (phone) => {
  ...
  await supabase.from('otp_verifications').upsert({ phone, otp, expires_at: expiresAt, attempts: 0 });
...
export const verifyOTP = async (phone, otp) => {
  const { data } = await supabase
    ...
    .maybeSingle();
  if (!data) return { success: false, message: 'Invalid or expired OTP' };
  return { success: true };
};
...
export const sendEmailOTP = async (email) => {
  ...
  await supabase.from('email_verifications').insert({ email, otp, expires_at: expiresAt, verified: false });
  return { success: true };
};
...
export const generateDriverQRCode = async (driverId, upiId) => {
  const qrData = JSON.stringify({ driverId, upiId, merchant: 'Maa Saraswati Travels', note: 'Ride Payment', timestamp: Date.now() });
  const qrCode = `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(qrData)}`;
  await supabase.from('drivers').update({ qr_code: qrCode, upi_id: upiId }).eq('id', driverId);
  return qrCode;
};

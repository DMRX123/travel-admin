// lib/supabase.js - Re-export from client
...
export const calculateFare = (vehicleType, distance, surgeMultiplier = 1) => {
  ...
  const minFares = { bike: 25, auto: 35, car: 50 };
...
export const completeRide = async (rideId, driverId, paymentId = null) => {
  try {
    const { data: ride } = await supabase.from('rides').select('fare').eq('id', rideId).single();
    ...
    await supabase.from('drivers').update({ is_online: true, current_ride_id: null }).eq('id', driverId);
...
export const cancelRide = async (rideId, userId, reason, cancelledBy = 'user') => {
  try {
    ...
    await supabase
      ...
      .eq('id', rideId);
    if (refundAmount > 0) {
      await supabase.from('refunds').insert({ ride_id: rideId, user_id: userId, amount: refundAmount, reason });
    }
    if (ride.driver_id) {
      await supabase.from('drivers').update({ is_online: true, current_ride_id: null }).eq('id', ride.driver_id);
    }
...
export const addLoyaltyPoints = async (userId, points, description) => {
  try {
    ...
    await supabase.from('loyalty_points').upsert({ user_id: userId, points: newPoints, tier });
    await supabase.from('loyalty_transactions').insert({ user_id: userId, points, type: 'earned', description });
...
export const updateRideStreak = async (userId) => {
  try {
    ...
    await supabase.from('ride_streaks').upsert({ user_id: userId, current_streak: currentStreak, best_streak: bestStreak });
...
export const sendOTP = async (phone) => {
  ...
  await supabase.from('otp_verifications').upsert({ phone, otp, expires_at: new Date(Date.now() + 5 * 60000).toISOString() });
...
export const generateDriverQRCode = async (driverId, upiId) => {
  const qrData = JSON.stringify({ driverId, upiId, merchant: 'Maa Saraswati Travels' });
...
export const getAvailableVehicles = async () => {
  try {
    const { data } = await supabase.from('vehicle_types').select('*').eq('is_active', true).order('sort_order');
